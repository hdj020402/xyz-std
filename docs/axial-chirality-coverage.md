...
`_order_h_on_heavy_atom` 对含有 2 个 H 的重原子使用 CIP 方法排序。当重原子是 `=CH2` 且双键 partner 是 sp 碳时，进入联烯/累积烯烃专用路径 `_order_2h_cumulene`。
...
所有轴手性类型中，**只有联烯/累积烯烃（含杂原子类似物）的轴穿过 2H 中心**，需要专用处理，已通过 `_order_2h_cumulene` 覆盖。其余类型的 2H 中心均在轴的取代基上，由现有的 sp2/sp3 CIP 路径自动处理。
